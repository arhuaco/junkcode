...
class TSearcher{
 ...
 char *
 TSearcher::MoveToStr(int *Move,char *Str){
   int First= *Move;
   int i=0;
   while(*Move){
     ++Move;
   ...
   sprintf(Str,"%d%c%d",SquareToNotation(First),i==2?'-':'x',
                         SquareToNotation(*(--Move)));
   return Str;
 }


 int
 TSearcher::SquareToNotation(int Sq){
   int n= 45-Sq;
   if(Sq<36)n--;
   if(Sq<27)n--;
   if(Sq<18)n--;
 ...
 void
 TSearcher::MakeMove(int *Board, int *Move, int *Save){
 ...
 void
 TSearcher::UndoMove(int *Board, int *Move, int *Save){
   do{
     Board[*(Move++)]= *(Save++);
   }while(*Move);
 }
 ...
 void
 TSearcher::CopyMove( int *Dest, int *Src){
   do{
     *(Dest++)=*(Src++);
   }while(*Src);
   *Dest=0;
 ...
 int
 TSearcher::AlphaBeta(int *Board, int Depth, TPlayer Player, int Lowest, int Highest,
              int RealDepth){
   NNodes++;
   ...
   /*
    * Note that if the other player has 0 moves we won!
   ...
   */
   ...
   for(int i=0;i<MoveList.NMoves&&Best<Highest;++i){
     ...
     int Score=-AlphaBeta(Board, Depth-1,Other(Player),-Highest, -Lowest,
                          RealDepth+1);
 ...
 TMoveInfo
 TSearcher::AlphaBeta0(int *Board, int Depth, TPlayer Player){
   int Lowest= -INFINITY;
 ...
 void
 TSearcher::PrintMoveInfo(const TMoveInfo &Info, bool OnlyMove){
 ...
 bool
 TSearcher::IsOnlyMove(TMoveInfo &Info, int *Board, TPlayer Player){
   TMoveList &MoveList= MoveListArray[0];
   TMoveGenerator.MakeMoveList(Board,Player,&MoveList);
 ...
 void
 TSearcher::GetComputerMove(int *Board, TPlayer Player, int *Move){
   TMoveInfo MoveInfo;
   MaxDepth=0;
   NEvaluated=0;
   NNodes=0;
