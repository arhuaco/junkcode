...
static struct fullex_dev
{
  ...
  int    is_open;           /* ¿Estamos usando este device? */
...
static int device_open(struct inode *inode, struct file *filep)
{
  ...
  /* Esto hace que no sea posible hacer llamadas de seek para el
   * dispositivo. Para complementar esto, se usa no_llseek en la
  ...
  if (dev->is_open) /* note que el próximo fullex_reset deja a dev->is_open en 0 */
    return -EBUSY;
  ...
  if ((filep->f_flags & O_ACCMODE) == O_WRONLY)
  {
    if (down_interruptible (&dev->sem))
      return -ERESTARTSYS;
    /* Este retorno no llega al espacio del usuario. Si toca, llega EINTR */
                           ...
                           loff_t * offset)
{
  ...
  if (dev->buff[pos] == 0 || pos >= dev->buff_size) /* retornar EOF */
  {
    ret_val = 0;
    goto read_exit;
  }
            ...
            unsigned int cmd, unsigned long arg)
{
  ...
  switch(cmd)
  {
    ...
    case FULLEX_SETSIZE:   /* SET: arg apunta al valor */
      ...
      if (!ret_val && dev->buff_size != new_size)
...
static struct file_operations fullex_file_operations = {
  ...
  .llseek   = no_llseek, /* está en linux/fs.h */
